...
with filenames \texttt{\$M3DC1\_CODE\_DIR/unstructured/*.mk}.  For most of
these systems, environment modules are also included in the
repository.  Current available \texttt{*.mk}:

\begin{description}
	\item{ kairos.mk}, ARCH=kairo
	\item{ marconi.mk}, ARCH=marconi
	\item{ mit.mk}, ARCH=mit
	\item{ perlmutter\_cpu.mk}, ARCH=perlmutter\_cpu
	\item{ perlmutter\_cpu\_gcc.mk}, ARCH=perlmutter\_cpu\_gcc
	\item{ perlmutter\_gpu.mk}, ARCH=perlmutter\_gpu
	\item{ saturn.mk}, ARCH=saturn
	\item{ sdumont.mk}, ARCH=sdumont
	\item{ stellar.mk}, ARCH=stellar
	\item{ fllux.mk}, ARCH=flux
\end{description}


These modules will load the appropriate software modules
...
\texttt{make OPT=1 3D=1 MAX\_PTS=60 ST=1 PAR=1 ARCH=\$M3DC1\_ARCH} & Builds the kinetic  version\\
