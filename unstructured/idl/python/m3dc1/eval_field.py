...
from .eval import eval as eval_poly
                ...
                for ii in range(i0, i1 + 1):
                    ...
                    if _is_in_tri_numba(lp0, lp1, a, b, c):
                        if wall_mask and izone != 2.0:
                            ...
                            result[ii, j] = _eval_poly_numba(f, lp0, lp1, lp2, t, i, operation)
                            out_mask[ii, j] = 0.0
...
def eval_field(
    ...
    if workers is None:
        w_env = os.getenv("M3DC1_EVAL_WORKERS", "").strip()
        if w_env:
            try:
                workers = int(w_env)
            except ValueError:
    ...
    if _NUMBA_AVAILABLE:
        ...
        for i in range(nelms):
            ...
            if threed:
                ...
                phi_elm = i_data[ib + 2]
                localphi_i = local_phi - phi_elm
                if localphi_i < 0 or localphi_i > dphi:
                    if mesh.nplanes > 0:
                        skip = nelms // mesh.nplanes
                        if skip > 0:
                            i += skip - 1
                    continue

            a, b, c, t, x0, y0 = i_data[0:6]
            if version >= 16 and ib < i_data.size:
                izone = i_data[ib]
            ...
            minpos = np.array([min(p1[0], p2[0], p3[0]), min(p1[1], p2[1], p3[1])], dtype=float)
            maxpos = np.array([max(p1[0], p2[0], p3[0]), max(p1[1], p2[1], p3[1])], dtype=float)

            if maxpos[0] < xrange[0] - xzero or maxpos[1] < yrange[0] - zzero:
                continue
            ...
            if p == 1:
                ...
                i0 = max(0, int(np.floor((minpos[0] - xrange[0] + xzero) / dx)))
                i1 = min(p - 1, int(np.ceil((maxpos[0] - xrange[0] + xzero) / dx + small)))
                j0 = max(0, int(np.floor((minpos[1] - yrange[0] + zzero) / dy)))
                j1 = min(p - 1, int(np.ceil((maxpos[1] - yrange[0] + zzero) / dy + small)))
                grid_i = range(i0, i1 + 1)
                grid_j = range(j0, j1 + 1)
            ...
            for j in grid_j:
                ...
                for ii in grid_i:
                    px = xi[ii] - xzero
                    localpos = np.array(
                        [(px - x0) * co + (py - y0) * sn - b, -(px - x0) * sn + (py - y0) * co, localphi_i],
                        dtype=float,
                    )
                    if is_in_tri(localpos, a, b, c):
                        if wall_mask and izone != 2:
                            result[ii, j] = 0.0
                        else:
                            result[ii, j] = eval_poly(f, localpos, t, i, operation=operation)
                            out_mask[ii, j] = 0.0
