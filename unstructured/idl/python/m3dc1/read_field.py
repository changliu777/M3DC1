...
def read_field(
    ...
    *,
    ...
    del mesh, h_symmetry, v_symmetry, dpsi, dimensions_out, flux_average, tpoints, logical, map_r, map_z, is_nonlinear, outval
    ...
    # average/sum/diff over file list
    if average or sum or diff or len(files) > 1:
        vals = []
        for i, f in enumerate(files):
            ...
            vals.append(r)
        base = vals[0]
        if diff and len(vals) >= 2:
            data = np.asarray(vals[0].data) - np.asarray(vals[1].data)
        else:
    ...
    slice_idx = int(slices if slices is not None else t)
    if last:
        slice_idx = nt - 1
    if equilibrium:
        if ilin == 1:
    ...
    if complex:
        ...
        if primitive is not None:
            ...
            if n == "zero":
                base = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                out = FieldResult(
                ...
                psi_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                ti = read_field("ti", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                out = ti
            elif n in {"vt_i", "vti"}:
                ti = read_field("ti", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(v0=1)
                ...
                p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                cs = read_field("cs", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                v_f = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                chi_f = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                pe = read_field("pe", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(p0=1)
                out = FieldResult(
                ...
                lp = read_field(psi_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=7, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                    psir = read_field(psi_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                iz = read_field(i_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(iz.r, iz.z) if itor == 1 else np.ones_like(np.asarray(iz.data), dtype=float)
                data = -np.asarray(iz.data) / rmat
                if ntor != 0:
                    psi_r = read_field(psi_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    fp_z = read_field(fp_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    if n == "jx_plasma":
                ...
                ir = read_field(i_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(ir.r, ir.z) if itor == 1 else np.ones_like(np.asarray(ir.data), dtype=float)
                data = np.asarray(ir.data) / rmat
                if ntor != 0:
                    psi_z = read_field(psi_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    fp_r = read_field(fp_name, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    if n == "jz_plasma":
                ...
                psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, wall_mask=wall_mask, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, wall_mask=wall_mask, cgs=cgs, mks=mks, return_meta=True)
                ...
                jy = read_field("jy", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, wall_mask=wall_mask, cgs=cgs, mks=mks, return_meta=True)
                eta = read_field("eta", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, wall_mask=wall_mask, cgs=cgs, mks=mks, return_meta=True)
                ...
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                chi_f = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                phi_lp = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, operation=7, cgs=cgs, mks=mks, return_meta=True)
                phi_r = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, operation=2, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi_f = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                f_f = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                pe0 = read_field("pe", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                n0 = read_field("ne", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                if linear and isubeq == 1 and slice_idx >= 0:
                    pe1 = read_field("pe", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    n1 = read_field("ne", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                p_f = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                v = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=operation, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ivform = int(read_parameter("ivform", filename=filename))
                if ivform == 0:
                ...
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                v_f = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                ...
                b = read_field("b", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(v0=1)
                ...
                den_mks = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, mks=True, cgs=False, return_meta=True)
                te_mks = read_field("te", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, mks=True, cgs=False, return_meta=True)
                ...
                jr0 = read_field("jx", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                jphi0 = read_field("jy", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                jz0 = read_field("jz", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                jr1 = read_field(f"jx{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                jphi1 = read_field(f"jy{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                jz1 = read_field(f"jz{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                br0 = read_field("bx", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                bphi0 = read_field("by", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                bz0 = read_field("bz", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                br1 = read_field(f"bx{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bphi1 = read_field(f"by{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz1 = read_field(f"bz{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                p1 = read_field(f"p{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                pr1 = read_field(f"p{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, operation=2, cgs=cgs, mks=mks, return_meta=True)
                pz1 = read_field(f"p{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, operation=3, cgs=cgs, mks=mks, return_meta=True)
                xir = read_field(f"xi_x{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                xiphi = read_field(f"xi_y{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                xiz = read_field(f"xi_z{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                vn = read_field(f"vn{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                pert = "_i" if n.endswith("_i") else ""
                vx = read_field(f"vx{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                g = np.asarray(read_gamma(filename=filename), dtype=float).reshape(-1)
                gamma = float(g[0]) if g.size else 0.0
                ...
                pert = "_i" if n.endswith("_i") else ""
                vy = read_field(f"vy{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                g = np.asarray(read_gamma(filename=filename), dtype=float).reshape(-1)
                gamma = float(g[0]) if g.size else 0.0
                ...
                pert = "_i" if n.endswith("_i") else ""
                vz = read_field(f"vz{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                g = np.asarray(read_gamma(filename=filename), dtype=float).reshape(-1)
                gamma = float(g[0]) if g.size else 0.0
                ...
                pert = "_i" if n.endswith("_i") else ""
                te1 = read_field(f"te{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                te0_r = read_field("te", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, operation=2, cgs=cgs, mks=mks, return_meta=True)
                te0_z = read_field("te", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, operation=3, cgs=cgs, mks=mks, return_meta=True)
                psi0 = read_field("psi", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                psi0_r = read_field("psi", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, operation=2, cgs=cgs, mks=mks, return_meta=True)
                psi0_z = read_field("psi", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, operation=3, cgs=cgs, mks=mks, return_meta=True)
                ...
                pert = "_i" if n.endswith("_i") else ""
                xi = read_field(f"displacement{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi0 = read_field("psi", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                xi = read_field(f"displacement{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi0 = read_field("psi", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                p0 = read_field("p", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                v_f = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                    rmat = radius_matrix(phi_f.r, phi_f.z)
                else:
                    rmat = np.ones_like(np.asarray(phi_f.data), dtype=float)
                ...
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                if n == "omega_*":
                    p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    num = s_bracket(p.data, psi.data, psi.r, psi.z)
                elif n == "omega_*i":
                    p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    pe = read_field("pe", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                om = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                omi = read_field("omega_*i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(t0=-1)
                ...
                rho_i = read_field("rho_i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                omega_ci = read_field("omega_ci", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                vstar = read_field("v_star", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                ...
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                i0 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                p0 = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                ...
                if not linear:
                    ...
                    psi1 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    i1 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    p1 = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    if complex:
                        f1 = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=True, cgs=cgs, mks=mks, return_meta=True)
                        f1p = 1j * float(ntor) * np.asarray(f1.data)
                    else:
                        f1p = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, operation=21, cgs=cgs, mks=mks, return_meta=False)
                ...
                if not linear:
                    ...
                    if complex:
                        ...
                        f1p = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, operation=21, cgs=cgs, mks=mks, return_meta=False)
                        p1p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, operation=21, cgs=cgs, mks=mks, return_meta=False)
                    bb = s_bracket(np.asarray(psi1.data), np.asarray(psi0.data), psi0.r, psi0.z) / np.maximum(rmat**2, np.finfo(float).tiny) + np.asarray(i1.data) * np.asarray(i0.data) / np.maximum(rmat**2, np.finfo(float).tiny) + a_bracket(np.asarray(psi0.data), np.asarray(f1p), psi0.r, psi0.z) / np.maximum(rmat, np.finfo(float).tiny)
                    pp = s_bracket(np.asarray(p0.data), np.asarray(p1.data), psi0.r, psi0.z)
                ...
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                v = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                ...
                va = read_field("va", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                eta = read_field("eta", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                eta_data = np.asarray(eta.data)
                d = dimensions()
                out = FieldResult(
                ...
                u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                v = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                omega_f = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                ...
                if n == "omega_e":
                    if int(read_parameter("ivform", filename=filename)) == 0:
                        data = np.asarray(omega_f.data) - (db * np.asarray(read_field("jy", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks))) / np.maximum(np.asarray(den.data) * np.maximum(rmat, np.finfo(float).tiny), np.finfo(float).tiny)
                    else:
                        data = np.asarray(omega_f.data) - (db * np.asarray(read_field("jy", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks))) / np.maximum(np.asarray(den.data), np.finfo(float).tiny)
                    d = dimensions(t0=-1)
                    sym = "$\\omega_e$"
                elif n == "ve_omega":
                    psi_lp = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=7, cgs=cgs, mks=mks, return_meta=True)
                    data = (
                    ...
                    ti = read_field("ti", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    btot = read_field("b", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    data = db * np.sqrt(np.maximum(2.0 * np.asarray(ti.data), 0.0)) / np.maximum(np.asarray(btot.data), np.finfo(float).tiny)
                    d = dimensions(l0=1)
                    sym = "$\\rho_i$"
                out = FieldResult(data=data, symbol=sym, units=parse_units(d, cgs=cgs, mks=mks), dimensions=d, r=omega_f.r, z=omega_f.z, time=omega_f.time, mask=omega_f.mask)
            elif n == "xbdotgradt":
                te0 = read_field("te", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(te0.r, te0.z) if itor == 1 else np.ones_like(np.asarray(te0.data), dtype=float)
                if ilin == 1:
                    te1 = read_field("te", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    psi1 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    if ntor != 0:
                        i1 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                        i0 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                        f1 = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                ...
                if ilin == 1:
                    p1 = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    psi1 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    if ntor != 0:
                        i0 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                        f1 = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                ...
                pert = "_i" if n.endswith("_i") else ""
                v_omega = read_field(f"v_omega{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                v_k_n = read_field(f"v_k_n{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field(f"i{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                ...
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                w = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                i0 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(phi_f.r, phi_f.z) if itor == 1 else np.ones_like(np.asarray(phi_f.data), dtype=float)
                ...
                den0 = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                if n == "chi_perp":
                    kappa = read_field("kappa", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    visc = read_field("visc", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                minor_r = read_field("r", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(psi.r, psi.z) if itor == 1 else np.ones_like(np.asarray(psi.data), dtype=float)
                ...
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(phi_f.r, phi_f.z) if itor == 1 else np.ones_like(np.asarray(phi_f.data), dtype=float)
                if int(read_parameter("ivform", filename=filename)) == 0:
                    data = (
                ...
                chi_r = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=2, cgs=cgs, mks=mks, return_meta=True)
                phi_z = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=3, cgs=cgs, mks=mks, return_meta=True)
                ...
                chi_z = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=3, cgs=cgs, mks=mks, return_meta=True)
                phi_r = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=2, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi0_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=2, cgs=cgs, mks=mks, return_meta=True)
                psi0_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=3, cgs=cgs, mks=mks, return_meta=True)
                psi_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=2, cgs=cgs, mks=mks, return_meta=True)
                psi_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=3, cgs=cgs, mks=mks, return_meta=True)
                ...
                if ntor != 0:
                    f_r = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=2, cgs=cgs, mks=mks, return_meta=True)
                    f_z = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=3, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi0_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=2, cgs=cgs, mks=mks, return_meta=True)
                psi0_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=3, cgs=cgs, mks=mks, return_meta=True)
                psi_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                psi_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(psi_r.r, psi_r.z) if itor == 1 else np.ones_like(np.asarray(psi_r.data), dtype=float)
                ...
                if ntor != 0:
                    fp_r = read_field("fp", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                    fp_z = read_field("fp", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi_lp = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=7, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                i_r = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                i_z = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(psi_r.r, psi_r.z) if itor == 1 else np.ones_like(np.asarray(psi_r.data), dtype=float)
                ...
                f_r = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                f_z = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi0_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=3, cgs=cgs, mks=mks, return_meta=True)
                i0 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                i0_r = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=2, cgs=cgs, mks=mks, return_meta=True)
                i0_z = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=3, cgs=cgs, mks=mks, return_meta=True)

                if n == "jpar_b":
                    psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                    jphi = read_field("jphi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    i1 = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    if n == "jb":
                        ...
                        if n == "jpar_lin":
                            ...
                            psi0_lp = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=7, cgs=cgs, mks=mks, return_meta=True)
                            b0b1 = (
                ...
                psi0_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=2, cgs=cgs, mks=mks, return_meta=True)
                psi0_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=3, cgs=cgs, mks=mks, return_meta=True)
                i_r = read_field(f"i{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                i_z = read_field(f"i{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                if ntor != 0:
                    psi_r = read_field(f"psi{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    psi_z = read_field(f"psi{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    fp_r = read_field(f"fp{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    fp_z = read_field(f"fp{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                denm = read_field("denm", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                denm_data = np.asarray(denm.data)
                base = denm
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                f_p = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, operation=11, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(den.r, den.z)
                bp2 = (
                ...
                q = read_field(qname, filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ex = read_field("e_r", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ey = read_field("e_phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ez = read_field("e_z", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bx0 = read_field("bx", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                by0 = read_field("by", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz0 = read_field("bz", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                p_p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, operation=11, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                f_p = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, operation=11, cgs=cgs, mks=mks, return_meta=True)
                kappar = float(read_parameter("kappar", filename=filename))
                ...
                p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                phi_f = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                if rvector:
                    ...
                    omega = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    v = np.sqrt(
                        np.maximum(
                ...
                i0_r = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=2, cgs=cgs, mks=mks, return_meta=True)
                i0_z = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, operation=3, cgs=cgs, mks=mks, return_meta=True)
                w0 = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                f_f = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                w_r = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                w_z = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                chi_lp = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=7, cgs=cgs, mks=mks, return_meta=True)
                chi_r = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                chi_z = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(psi.r, psi.z) if itor == 1 else np.ones_like(np.asarray(psi.data), dtype=float)
                rfac = 1j * ntor
                data = (
                    -(
                ...
                eta = read_field("eta", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                f_f = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(psi.r, psi.z) if itor == 1 else np.ones_like(np.asarray(psi.data), dtype=float)
                ...
                force_phi = read_field("force_phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                bx = read_field("bx", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                by = read_field("by", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz = read_field("bz", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                jx = read_field("jx", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                jz = read_field("jz", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                jy = read_field("jy_plasma", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                if n == "jxb_x":
                ...
                rfac = 1j * ntor
                if n == "torque_p":
                    p = read_field("p", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    psi_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                    psi_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                    i_r = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                    i_z = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                    f_r = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=2, cgs=cgs, mks=mks, return_meta=True)
                    f_z = read_field("f", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, operation=3, cgs=cgs, mks=mks, return_meta=True)
                    rmat = radius_matrix(psi_r.r, psi_r.z) if itor == 1 else np.ones_like(np.asarray(psi_r.data), dtype=float)
                    if n == "torque_b2":
                        data = (
                ...
                rfac = 1j * ntor
                if n == "torque_mu":
                    mu = read_field("visc", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    mu_c = read_field("visc_c", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    mu_c_data = np.asarray(mu_c.data)
                    w = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    if ntor != 0:
                        u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                        chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                        data = (
                            data
                    ...
                    w0 = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    n0 = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    w = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    psi0 = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    n0 = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, linear=linear, cgs=cgs, mks=mks, return_meta=True)
                    u = read_field("phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    w = read_field("omega", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                    chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, cgs=cgs, mks=mks, return_meta=True)
                ...
                v = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(v.r, v.z) if itor == 1 else np.ones_like(np.asarray(v.data), dtype=float)
                ivform = int(read_parameter("ivform", filename=filename))
                ...
                v = read_field("v", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                rmat = radius_matrix(v.r, v.z) if itor == 1 else np.ones_like(np.asarray(v.data), dtype=float)
                ivform = int(read_parameter("ivform", filename=filename))
                ...
                chi = read_field("chi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                psi = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                b = read_field("i", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                pe = read_field("pe", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                den = read_field("den", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                eta = read_field("eta", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                eta_data = np.asarray(eta.data)
                ...
                if not zvector and not yvector:
                    ephi_z = read_field(f"e_phi{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    ez_phi = read_field(f"e_r{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=11, phi=phi, cgs=cgs, mks=mks, return_meta=False)
                if not rvector and not yvector:
                    ephi_r = read_field(f"e_phi{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    er_phi = read_field(f"e_r{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=11, phi=phi, cgs=cgs, mks=mks, return_meta=False)
                if not rvector and not zvector:
                    ez_r = read_field(f"e_z{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    er_z = read_field(f"e_r{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                pert = "_i" if n.endswith("_i") else ""
                er_r = read_field(f"e_r{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=2, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ephi_phi = read_field(f"e_phi{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=11, phi=phi, cgs=cgs, mks=mks, return_meta=False)
                ez_z = read_field(f"e_z{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, operation=3, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(potential=1, l0=-2)
                ...
                ey = read_field("e_phi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ez = read_field("e_z", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bx = read_field("bx", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                by = read_field("by", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz = read_field("bz", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                ex = read_field("e_r", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ez = read_field("e_z", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bx = read_field("bx", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                by = read_field("by", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz = read_field("bz", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                rho = read_field("r", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                bp = read_field("bp", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                bt = read_field("bt", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                ...
                b2 = read_field("b2", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(b0=1)
                ...
                out = read_field("bp", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
            elif n in {"toroidal field strength"}:
                out = read_field("bt", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
            elif n in {"toroidal field", "by", "bt"}:
                I = read_field("I", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, complex=False, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                psi_z = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                fp_r = read_field("fp", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=False)
                ...
                psi_r = read_field("psi", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                fp_z = read_field("fp", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=False)
                rmat = radius_matrix(psi_r.r, psi_r.z) if itor == 1 else np.ones_like(np.asarray(psi_r.data), dtype=float)
                d = dimensions(b0=1)
                ...
                psi_r = read_field("psi_plasma", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=2, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                fz = read_field("f_plasma", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, operation=3, linear=linear, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                ...
                bx = read_field("bx", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz = read_field("bz", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(b0=1)
                ...
                bx = read_field("bx", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                by = read_field("by", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                bz = read_field("bz", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=linear, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                d = dimensions(b0=2)
                ...
                te0 = read_field("te", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, equilibrium=True, cgs=cgs, mks=mks, return_meta=True)
                pert = "_i" if n.endswith("_i") else ""
                psi = read_field(f"psi{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, cgs=cgs, mks=mks, return_meta=True)
                i_f = read_field(f"i{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, cgs=cgs, mks=mks, return_meta=True)
                f_f = read_field(f"f{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, cgs=cgs, mks=mks, return_meta=True)
                te1 = read_field(f"te{pert}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, cgs=cgs, mks=mks, return_meta=True)
                if itor == 1:
                    rmat = radius_matrix(psi0.r, psi0.z)
                else:
                    rmat = np.ones_like(np.asarray(psi0.data), dtype=float)
                ...
                if linear:
                    by0 = read_field("by", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    b0 = read_field(f"b{axis}", filename=filename, slices=-1, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    by1 = read_field("by", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    b1 = read_field(f"b{axis}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, linear=True, complex=complex, phi=phi, cgs=cgs, mks=mks, return_meta=True)
                    ...
                    by = read_field("by", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
                    b = read_field(f"b{axis}", filename=filename, slices=slice_idx, points=points, rrange=rrange, zrange=zrange, cgs=cgs, mks=mks, return_meta=True)
